 ...
 */
void SHA1Transform(UINT32 state[5], const BYTE buffer[SHA1_BLOCK_LENGTH])
{
	UINT32 a, b, c, d, e;
	BYTE workspace[SHA1_BLOCK_LENGTH];
 ...
 */
void SHA1Init(PSHA1_CTX context)
{
 ...
 */
void SHA1Update(PSHA1_CTX context, PCBYTE data, UINT len)
{
	UINT i, j;

	j = (UINT)((context->count >> 3) & 63);
	context->count += (len << 3);
	if ((j + len) > 63) {
		(void)memcpy(&context->buffer[j], data, (i = 64-j));
		SHA1Transform(context->state, context->buffer);
		for ( ; i + 63 < len; i += 64)
			SHA1Transform(context->state, (BYTE *)&data[i]);
		j = 0;
	} else {
		i = 0;
	}
 ...
 */
void SHA1Pad(PSHA1_CTX context)
{
	BYTE finalcount[8];
	UINT i;
