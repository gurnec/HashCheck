...
void MD4Pad(PMD5_CTX);
void MD4Transform(UINT32 [4], const BYTE [MD4_BLOCK_LENGTH]);

/*
 * Start MD4 accumulation.
 * Set bit count to 0 and buffer to mysterious initialization constants.
 */
void MD4Init(PMD4_CTX ctx)
{
	ctx->count = 0;
 ...
 * of bytes.
 */
void MD4Update(PMD4_CTX ctx, PCBYTE input, UINT len)
{
	UINT have, need;

	/* Check how many bytes we already have and how many more we need. */
	have = (UINT)((ctx->count >> 3) & (MD4_BLOCK_LENGTH - 1));
 ...
 */
void MD4Transform(UINT32 state[4], const BYTE block[MD4_BLOCK_LENGTH])
{
	UINT32 a, b, c, d, in[MD4_BLOCK_LENGTH / 4];
